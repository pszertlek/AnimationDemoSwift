...
class DynamicCollectionViewLayout: UICollectionViewFlowLayout {
    ...
    override func prepareLayout() {
        ...
        var itemVisibleIndexPath = super.layoutAttributesForElementsInRect(visibleRect)
        let itemVisibleIndexPathSet = NSMutableSet(array: itemVisibleIndexPath!).valueForKeyPath("indexPath")!
        let noLongerVisibleBeviours = self.dynamicAnimator.behaviors.filter { (behaviour:UIDynamicBehavior) -> Bool in
            ...
            let indexPath = itemVisibleIndexPathSet.member(attribute.indexPath) != nil
            return !indexPath
        ...
        for item in noLongerVisibleBeviours {
            ...
            self.visibleIndexPathSet.removeObject(attribute.indexPath)
        }
